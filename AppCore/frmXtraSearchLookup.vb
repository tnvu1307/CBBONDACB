...
Public Class frmXtraSearchLookup
...
#Region " Khai báo hằng, biến "
    ...
    Public mv_SearchData As DataSet
    ...
    Public mv_Selected_data As DataTable
...
#Region " Overridable Function "
                    ...
                    If Not SearchGridView.GetFocusedDataRow() Is Nothing Then
                        If SearchGridView.FocusedRowHandle >= 0 Then
                            ...
                            If Len(mv_strRefColumn) > 0 Then
                                RefValue = SearchGridView.GetFocusedDataRow().Item(mv_strRefColumn).ToString
                            Else
    ...
    Protected Overridable Function OnSearch(Optional ByVal pv_strIsLocal As String = "", Optional ByVal pv_strModule As String = "", Optional ByVal page As Int32 = 1) As Int32
        ...
        Try
            ...
            Else 'Command text. Only for defaul condition
                ...
                If mv_ISFLTCODEID = "Y" Then
                    ...
                    If mv_strTellerId <> ADMIN_ID Then
                        ...
                        v_strFUNDFilter &= " AND  FUNDCODEID IN ( " & v_strSQL & " ) "
                    End If
                ...
                If (pv_strIsLocal <> "") And (pv_strModule <> "") Then
                    ...
                    If SearchByTransact = True Then
                        strRow = strRow.Replace("<$MBID>", HO_MBID)
                    Else
                        strRow = strRow.Replace("<$MBID>", Me.BranchId)
                    End If
                    'TheNN sua
                    ...
                    If Me.bciCheckAll.Checked = True Then
                        ...
                        SearchGrid.DataSource = mv_SearchData.Tables("ObjData")
            ...
            XtraGridFormat(gvResult, c_ResourceManager & UserLanguage, mv_arrSrFieldSrch, mv_arrSrFieldFormat, mv_arrSrFieldWidth, mv_arrSrFieldDisp, mv_arrSrFieldDisplay)
    ...
    Private Sub gridView_PopupMenuClick(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs)
        ...
        If e.MenuType = GridMenuType.Row Then
            ...
            For Each objKey In v_hashTLTX.Keys
                ...
                item = New DevExpress.Utils.Menu.DXMenuItem(v_strTLTXCD.Trim() & "." & v_strTXNAME.Trim())
    ...
    Function GetExecuteTLTXCD() As Hashtable
        ...
        Dim itemField, v_strTXNAME, v_strFLDCODE, v_strValue As String
        Try
                            ...
                            If Not gvSearchSelection.GetSelectedRow(v_intRow) Is Nothing Then
                                ...
                                For Each objKey In mv_arrStrTLTXNAME.Keys
                                    ...
                                    For v_intCol = 0 To gvResult.Columns.Count - 1
                                        ...
                                        If v_strFLDCODE <> "CheckMarkSelection" And v_strFLDCODE <> "STT" Then
    ...
    Protected Overridable Function OnExecute(Optional ByVal v_strRefTLTXCD As String = vbNullString) As Int32
        ...
        Dim v_strISTRANSVIEW As String = "N"
        'Which TLTXCD is OK
        If Me.mv_strTLTXCD IsNot Nothing Then
            If Not mv_strTLTXCD.IndexOf("/") > 0 Then
                ...
                v_strSQL = "SELECT APPMODULES.MODCODE, SEARCH.TLTXCD, SEARCHFLD.FIELDCODE, SEARCHFLD.FLDCD, SEARCHFLD.FIELDTYPE, SEARCH.ISTRANSVIEW " & ControlChars.CrLf _
            ...
            Else
                If v_strRefTLTXCD IsNot Nothing Then
                    v_strSQL = "SELECT APPMODULES.MODCODE, SEARCHTXMAPFLD.TLTXCD, SEARCHFLD.FIELDCODE, SEARCHTXMAPFLD.FLDCD, SEARCHFLD.FIELDTYPE, SEARCH.ISTRANSVIEW " & ControlChars.CrLf _
                ...
                Else
                    ...
                    If v_hashTLTX.Count > 0 Then
                        ...
                        Else
                            ...
                            'Dim frm = New frmXtraSearchLookup__ChooseTransaction(v_hashTLTX)
                            'frm.ShowDialog()
        ...
        If Not Me.bciExecuteAll.Checked Then
            If Not v_nodeList.Count = 0 Then
                ...
                If Not SearchGrid Is Nothing Then
                    If gvResult.RowCount > 0 Then
                        If mv_strTLTXCD = "9304" Then
                            ''Giao dich gom bang ke
                            Dim v_strAUTOID, v_strSETTLEMENTDATE, v_strTXTYPE, v_strFUNDACCOUNT, v_strSYMBOL As String
                            Dim l_beneficiaryaccount, l_beneficiaryname, l_beneficiarybank As String

                            v_strAUTOID = String.Empty
                            v_strSETTLEMENTDATE = String.Empty
                            v_strTXTYPE = String.Empty
                            v_strFUNDACCOUNT = String.Empty
                            v_strSYMBOL = String.Empty
                            l_beneficiaryaccount = String.Empty
                            l_beneficiaryname = String.Empty
                            l_beneficiarybank = String.Empty

                            Dim v_dblTXAMT As Double = 0
                            For v_intRow = 0 To gvSearchSelection.SelectedCount - 1
                                v_strAUTOID = v_strAUTOID & gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("AUTOID")) & "$"
                                v_strSETTLEMENTDATE = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("SETTLEMENTDATE"))
                                v_strTXTYPE = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("TXTYPECD"))
                                v_strFUNDACCOUNT = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("FUNDACCOUNT"))
                                v_strSYMBOL = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("SYMBOL"))
                                v_dblTXAMT = v_dblTXAMT + gf_CorrectNumericField(gvSearchSelection.GetSelectedRow(v_intRow)("TXAMT"))
                                l_beneficiaryaccount = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("BENEFICIARYACCOUNT"))
                                l_beneficiaryname = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("BENEFICIARYNAME"))
                                l_beneficiarybank = gf_CorrectStringField(gvSearchSelection.GetSelectedRow(v_intRow)("BENEFICIARYBANK"))

                            Next

                            v_strFLDDEFVAL = v_strFLDDEFVAL & "[02." & v_strSYMBOL & "][03." & v_strTXTYPE & "][12." & v_strSETTLEMENTDATE & "][13." & v_strFUNDACCOUNT & "][14." & v_strAUTOID & "][15." & l_beneficiaryaccount & "][16." & l_beneficiaryname & "][17." & l_beneficiarybank & "][10." & v_dblTXAMT & "]"
                            SetTransactForm()
                            v_strTLTXCD = "9304"
                            v_strMODCODE = "FA"
                            ...
                            mv_frmTransactScreen.mv_isReadOnly = (v_strISTRANSVIEW = "Y")
                            ...
                            For v_intRow = 0 To gvSearchSelection.SelectedCount - 1
                                ...
                                If Not gvSearchSelection.GetSelectedRow(v_intRow) Is Nothing Then
                                        ...
                                        For j = 0 To v_nodeList.Item(i).ChildNodes.Count - 1
                                            With v_nodeList.Item(i).ChildNodes(j)
                                                ...
                                                Select Case Trim(v_strFLDNAME)
                                                    ...
                                                    Case "ISTRANSVIEW"
                                                        v_strISTRANSVIEW = Trim(v_strValue)
                                                End Select
                                            End With
                                        Next
                                    ...
                                    If v_strMODCODE <> "" And v_strTLTXCD <> "" Then
                                        ...
                                        mv_frmTransactScreen.BusDate = Me.BusDate
                                        mv_frmTransactScreen.TellerName = Me.TellerName
                                        'Khong an truong voi giao dich 9238 goi tu GV
                                        If v_strTLTXCD = "9238" Then
                                            mv_frmTransactScreen.mv_isReadOnly = False
                                        Else
                                            mv_frmTransactScreen.mv_isReadOnly = (v_strISTRANSVIEW = "Y")
                                        End If
                                        If IIf(IsDBNull(Len(v_strPostingDate)), 0, Len(v_strPostingDate)) > 0 Then
    ...
    Protected Overridable Function OnViewTransaction() As Boolean
        Return False
    End Function
...
#Region " Other methods "
    ...
    Protected Overridable Sub InitDialog()
        ...
        For i = 0 To mv_arrStQuickSearch.Count() - 1
        ...
        sccSearch.Collapsed = mv_isQuickSearch
        ...
        'tiennv
        'Neu ma cac control da bi an thi an luon group cua no
        VisableRibbonPageGroup()


    End Sub

    Public Sub VisableRibbonPageGroup()
        Try

            If RibbonPage2.Groups Is Nothing Or RibbonPage2.Groups.Count = 0 Then
                RibbonPage2.Visible = False
            End If

            For Each g As RibbonPageGroup In RibbonPage2.Groups
                If g.ItemLinks Is Nothing Or g.ItemLinks.Count = 0 Then
                    g.Visible = False
                    Continue For
                End If

                Dim allItemVisibility As Boolean = False
                For Each item As DevExpress.XtraBars.BarButtonItemLink In g.ItemLinks
                    If item.Item.Visibility <> DevExpress.XtraBars.BarItemVisibility.Never Then
                        allItemVisibility = True
                    End If
                    g.Visible = allItemVisibility
                Next
            Next
        Catch ex As Exception
...
#Region " Các sự kiện của form "
    ...
    Private Sub Grid_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        Try
            'Select Case e.KeyCode
            '    Case Keys.Control.C
            '        Clipboard.SetDataObject(gvResult.GetRowCellValue(gvResult.FocusedRowHandle, gvResult.FocusedColumn).ToString())
            ...
            '        '    Dim colView As GridColumn
            '        '    Try
            '        '        colView = gvResult.Columns(col.ColumnName)
            '        '    Catch ex As Exception

            '        '    End Try
            '        '    If (colView IsNot Nothing) Then
            '        '        If (colView.Visible = True) Then
            '        '            columns.Add(col)
            '        '        End If
            '        '    End If
            '        'Next
            ...
            '        'Dim num As Integer
            '        'Dim selectedHanders = gvResult.GetSelectedRows
            '        'If (selectedHanders IsNot Nothing) Then
            '        '    For Each num In selectedHanders
            '        '        Dim rv As DataRowView = gvResult.GetRow(num)
            '        '        If (rv IsNot Nothing) Then
            '        '            For Each col As DataColumn In columns
            '        '                strToClipboard += rv.Row(col.ColumnName).ToString() + Convert.ToChar(9)
            '        '            Next
            '        '        End If
...
#Region " QuickSearch "
    Private Sub BuildQuickCriterial()
        ...
        If Me.lblQueryDate.Visible And Me.lblQueryDate.Checked Then
            ...
            v_dtFrom = "TO_DATE('" & Me.dtFrom.Text & "', 'DD/MM/YYYY')"
            v_dtTo = "TO_DATE('" & Me.dtTo.Text & "', 'DD/MM/YYYY')"
